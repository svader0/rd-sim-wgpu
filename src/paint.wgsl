// Paint shader for adding chemical B to the simulation
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Calculate distance from paint center
    let dx = f32(pos.x) - paint_params.center_x;
    let dy = f32(pos.y) - paint_params.center_y;
    let dist = sqrt(dx * dx + dy * dy);
    
    // If within radius, add chemical B (set V to 1.0), otherwise copy existing value
    if (dist <= paint_params.radius) {
